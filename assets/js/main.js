...
const cardValues = [
    ...
    false, false, false, false,
    false, false, true, true // true đại diện cho 'Jojo'
];
...
function createBoard() {
    ...
    shuffledValues.forEach((value, index) => {
        ...
        if (value === true) {
            cardFront.innerHTML = '<div class="default-front-card card-jojo"><img src="./assets/images/img_jojo.png"/></div>';
            cardFront.classList.add('isJojo'); // Giữ class 'isJojo' cho styling màu vàng
        } else {
            cardFront.innerHTML = '<div class="default-front-card"><img src="./assets/images/img_ice_cream.png"/></div>'; // Hiển
        }
        ...
        if (specialIndices.includes(index)) { // Check the current index (0-11)
...
function flipCard() {
    ...
    if (!gameStarted || lockBoard || hasWon || this.classList.contains('is-flipped')) return;
